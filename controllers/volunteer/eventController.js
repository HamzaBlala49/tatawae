...
const join = async (req, res) => {
  try {
    ...
    if (
      _event.volunteersNumber == _event.volunteers.length ||
      new Date(_event.startDate).getTime() < Date.now()
    ) {
...
const evaluationInfo = async (req, res) => {
  try {
    ...
    const id = req.params.id;
    const data = await event
      .findOne({ _id: id, "volunteers.volunteerId": user._id })
      .select("title image volunteers");
    const _data = {
      title: data.title,
      image: data.image,
    };
...
export { index, find, join, eventMember, evaluationInfo };
